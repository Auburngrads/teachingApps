...
sidebarLayout(
  sidebarPanel(width = 4,
    selectInput('correct', h2('True Distribution'), 
                choices = c('Green', 'Blue', 'Red'), 
                selected = 'Blue'),
    
    actionButton('mlesample',h4('sample 1')),
    actionButton('mlesample10',h4('sample 10')),
    actionButton('clear',h4('clear'))),
  
  mainPanel(plotOutput('plotmle', height = '650px'), width = 8)))),

server = function(input, output, session) {                

mle <- reactiveValues(dats = NULL, params = NULL, sims = 0, dat1 = NULL, dat2 = NULL, dat3 = NULL)
...
text(rep(69,3), c(0.025,0.0225, 0.02), rep('=',3), col = c('blue', 'darkgreen', 'red'), cex = 2, adj = 0, font = 2)
...
text(rep(72.5,3), c(0.025,0.0225, 0.02), c(round(sum(log(mle$dat1)), digits = 4), round(sum(log(mle$dat2)), digits = 4), round(sum(log(mle$dat3)), digits = 4)), col = c('blue', 'darkgreen', 'red'), cex = 2, adj = 0)
