...
heatexchanger_data6 <- function(theme = "flatly",  width = '100%', storyteller = F, css = NULL,
                            height = `if`(storyteller,'800px','600px'),
                            more.opts = list(NA),...) {

    dir <- dirname(system.file("apps", "heatexchanger_data6", "global.R", package = "teachingApps"))
