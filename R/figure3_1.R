...
shinyApp(options = list(height = '600px', width = '100%'),
    ...
    sidebarLayout(
      sidebarPanel(
        shinyAce::aceEditor("binplot", mode = "r", theme = "github", height = "450px",
...
text(x = rep(-35,6),
     ...
     c('Unconditional','Plant 1','100 tubes', 
       'Failure','at start', 'Probability'),
     cex=c(1,1.15,1,1,1,1))

text(x = c(seq(50,250,100),325),
     y = rep(20,4),
     c(expression(pi[1]),expression(pi[2]),
       expression(pi[3]),expression(pi[4])),
     cex=1.5)

text(x = seq(50,250,100),
     y = rep(108,3),
     c('Year 1','Year 2','Year 3'),cex=rep(1.15,3))

text(x = c(150,320,320),
     y = c(rep(140,2),130),
     c('Cracked tubes', 'Uncracked','tubes'),
     cex=rep(1,3))

text(x = c(seq(50,250,100),325),
     y = rep(77,4),
     c('1','2','2','95'),cex=rep(1.5,4))"),
              actionButton("evalbin", h4(HTML("<b>Evaluate</b>")))),
