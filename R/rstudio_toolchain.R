...
rstudio_toolchain <- function(theme = "flatly",  width = '100%', storyteller = F, css = NULL,
                            height = `if`(storyteller,'800px','600px'),
                            more.opts = list(NA),...) {

    dir <- dirname(system.file("apps", "rstudio_toolchain", "global.R", package = "teachingApps"))

    teachingApps::getPackage(pkg = 'codemirrorR', repo = 'timelyportfolio')
