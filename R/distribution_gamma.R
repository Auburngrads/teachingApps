...
distribution_gamma <- function(theme = "flatly",  width = '100%', storyteller = F, css = NULL,
                            height = `if`(storyteller,'800px','600px'),
                            more.opts = list(NA),...) {

    dir <- dirname(system.file("apps", "distribution_gamma", "global.R", package = "teachingApps"))
