...
repairable_nonrepairable <- function(theme = "flatly",  width = '100%', storyteller = F, css = NULL,
                            height = `if`(storyteller,'800px','600px'),
                            more.opts = list(NA),...) {

    dir <- dirname(system.file("apps", "repairable_nonrepairable", "global.R", package = "teachingApps"))
    

    teachingApps::getPackage(pkg = 'diagram')
