...
server = function(input, output, session) {
  ...
  output$sign <- renderUI({HTML(teachingApp('data_science'))})
  
  output$stats <- visNetwork::renderVisNetwork({
           
n.names <- list(
hub <- 'DATA ANALYSIS',
nodes <- c('Distribution\nFitting',
           'Linear\nModels',
           'Stochastic\nProcesses',
           'Confidence\nIntervals',
           'Information\nTheory',
...
nodes <- data.frame(id = 1:length(unlist(n.names)), 
                    ...
                    color = rep(c('orange','blue'), 
                                times = lapply(n.names, length)))

edges <- data.frame(from = 1, to = 2:length(unlist(n.names)))
...
output$coding <- visNetwork::renderVisNetwork({
...
output$munging <- visNetwork::renderVisNetwork({
...
output$reproduce <- visNetwork::renderVisNetwork({
...
output$webdev <- visNetwork::renderVisNetwork({
...
output$stat687 <- visNetwork::renderVisNetwork({
