...
output$overview <- renderUI({ teachingApps::add_rmd('background.Rmd') })
...
output$levfunc <- renderUI({
  ...
  teachingApps::add_rmd(funcs_lev)

})
output$levprops <- renderUI({
   props_lev <- system.file('apps','distribution_lev','lev-props.Rmd', package = 'teachingApps')
   teachingApps::add_rmd(props_lev)
})
   teachingApps::add_server('distribution_lev_functions', envir = environment())
