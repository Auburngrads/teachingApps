 ...
 tabPanel(h4('Global Ranking'),
    sidebarLayout(
       sidebarPanel(width = 4,
          ...
          selectInput("metric", 
                      label = h4("Choose a metric to display"),
                      choices = names(lpi)[c(3, 5:10)],
                      selected = "Overall LPI Score"),
                        
          radioButtons("range", 
                       label = h4("Choose a year to display"),
          ...
          checkboxInput("alldata", 
                        label = h4("Show all data in table"), 
                        value = FALSE),
                        
          downloadButton('downloadData', h4('Download Table')),
       ...
       mainPanel(width = 8,
        tabsetPanel(type = 'pills',
          tabPanel(h4('Interactive Map of Results'),
          ...
          tabPanel(h4('Full Table of Results'),  
            DT::dataTableOutput("mapTable")))))),
        
  tabPanel(h4('Country Score Card'),
    sidebarLayout(
       sidebarPanel(width = 3,
         helpText("Modify the map and data table by selecting desired information below."),
         selectInput("country", 
                     label = h4("Choose a country to display"),
                     choices = sort(unique(lpi$`Country`)),
                     selected = "United States"),
                         
          checkboxGroupInput('selectedYears', 
                             h4("Years to include in Radar Chart"),
                             seq(2010, 2016, by = 2),
