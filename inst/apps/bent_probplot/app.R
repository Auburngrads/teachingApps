...
args <- system.file('apps', 'bent_probplot', 'args.R', package = 'teachingApps')

shinyApp(options = list(height = '600px', width = '100%'),
         
ui = fluidPage(theme = shinythemes::shinytheme(theme = source(args)[[1]]$theme), 
...
server = function(input, output, session) {
  ...
  output$sign <- renderUI({HTML(teachingApps::teachingApp(source(args)[[1]]$appName))})
