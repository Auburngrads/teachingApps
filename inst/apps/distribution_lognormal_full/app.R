...
ui = navbarPage(windowTitle = 'Lognormal Distribution', 
...
tabPanel(h4('Distribution Functions'),
         mainPanel(uiOutput('lnorfunc'), class = 'shiny-text-output', width = 12)),

tabPanel(h4('Distribution Properties'),
         mainPanel(uiOutput('lnorprops', class = 'shiny-text-output'), width = 12)),
...
output$lnorfunc <- renderUI({ 
  withMathJax(HTML(includeMarkdown('lnor-func.Rmd')))
})
output$lnorprops <- renderUI({HTML(includeMarkdown('lnor-props.Rmd'))
}) 
})
